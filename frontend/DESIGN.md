...
- The running Docker API still contains its original bundled frontend. The redesigned source and production build are available locally; the running Vite preview at `http://127.0.0.1:5173/app/` uses the existing API. A subsequent normal image build picks up the redesign.
